...
export default class TestScene extends Phaser.Scene {
    ...
    create() {
        ...
        this.input.on("pointermove", (ev: Pointer) => {
            ...
            if (this.drawSize == 1) {
                this.s2.fillRect(40 + (i * this.squareSize), 40 + (j * this.squareSize),
                    this.squareSize, this.squareSize)
            } else if (this.drawSize == 2) {
                this.s2.fillRect(40 + ((i - 1) * this.squareSize), 40 + ((j -1) * this.squareSize),
                    this.squareSize, this.squareSize)
                this.s2.fillRect(40 + ((i - 1) * this.squareSize), 40 + ((j) * this.squareSize),
                    this.squareSize, this.squareSize)
                this.s2.fillRect(40 + ((i) * this.squareSize), 40 + ((j -1) * this.squareSize),
                    this.squareSize, this.squareSize)
                this.s2.fillRect(40 + ((i) * this.squareSize), 40 + ((j) * this.squareSize),
                    this.squareSize, this.squareSize)
            ...
            for (let j = 0; j < this.sectorSize; j++) {
                switch (this.sectorMap[i][j]) {
                    case SectorObjectMasks.OutOfBoundSpace:
                        ...
                        this.sectorObjectGraphics.fillRect(40 + (i * this.squareSize), 40 + (j * this.squareSize),
                            this.squareSize, this.squareSize)
                        break;
